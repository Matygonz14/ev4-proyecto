...
const UsersAdmin = () => {
        ...
        fetch(config.apiURL+"users/"+config.operatorId, requestOptions).then((response) => {
            ...
            if(userList.length === 0){
                ...
                rowData = userList.map(u => {
                    ...
                    return (<tr>
                        <td>{u.name}</td><td className="text-left">{u.nickname}</td><td className="text-left">{u.level}</td>
    ...
    const disable = (user) => {
        if(window.confirm("¿Está seguro/a de querer deshabilitar:\n"+user.name)){
            ...
            fetch(config.apiURL+"clients/"+user.id, requestOptionsPatch).then((response) => {
                return response.json();
            }).then((result) => {
                updateUsers();
